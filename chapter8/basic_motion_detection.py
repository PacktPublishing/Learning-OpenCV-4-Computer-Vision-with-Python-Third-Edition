...
while success:
    ...
    cv2.erode(diff, erode_kernel, diff, iterations=2)
    cv2.dilate(diff, dilate_kernel, diff, iterations=2)

    if OPENCV_MAJOR_VERSION >= 4:
        # OpenCV 4 or a later version is being used.
        contours, hier = cv2.findContours(diff, cv2.RETR_EXTERNAL,
        ...
        _, contours, hier = cv2.findContours(diff, cv2.RETR_EXTERNAL,
                                             cv2.CHAIN_APPROX_SIMPLE)
